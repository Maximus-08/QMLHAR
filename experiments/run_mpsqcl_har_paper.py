"""
Paper-compliant MPSQCL HAR training pipeline (Phase 1: Pre-training, Phase 2: Fine-tuning).
...
Matches:
  ...
  - Pre-training augmentation: resampling + negation only, with M positive views (default 4)
  - 64% / 16% / 20% splits
...
def pretrain(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (batch_views, _) in enumerate(train_loader):
            # batch_views is a tuple of M views of shape (batch, channels, time)
            views = [v.to(device) for v in batch_views]

            # Forward: process all views
            z_list = []
            for v in views:
                h = encoder(v)
                z = quantum_head(h)
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_encoder_path = os.path.join(
                ...
                f"mpsqcl_paper_encoder_best_{args.dataset}.pt",
            )
        ...
        if (epoch + 1) % args.save_every == 0 or (epoch + 1) == args.epochs:
            ckpt_path = os.path.join(
                os.path.dirname(args.output_file),
                f"mpsqcl_paper_checkpoint_epoch{epoch + 1}_{args.dataset}.pt",
            )
            torch.save(
                {
                    "epoch": epoch + 1,
    ...
    with open(results_path, "w") as f:
        f.write("=== MPSQCL HAR Paper Pre-Training Results ===\n")
...
def finetune(args):
    print("=" * 60)
    print("MPSQCL HAR Paper Version — Phase 2: Supervised Fine-Tuning")
    print("=" * 60)
    ...
    for epoch in range(args.epochs):
        ...
        if test_f1 > best_test_f1:
            ...
            best_model_path = os.path.join(
                os.path.dirname(args.output_file),
                f"mpsqcl_paper_best_model_{args.dataset}.pt",
            )
            torch.save(
                {
    ...
    os.makedirs(os.path.dirname(results_path), exist_ok=True)
    with open(results_path, "w") as f:
        f.write("=== MPSQCL HAR Paper Fine-Tuning Results ===\n")
        f.write(f"Dataset: {args.dataset}\n")
...
def main():
    ...
    default_output = os.path.join(project_root, "results", "mpsqcl_paper_results.txt")

    parser = argparse.ArgumentParser(description="MPSQCL HAR Paper Training Pipelines")
    parser.add_argument(
    ...
    parser.add_argument(
        "--n_views", type=int, default=4, help="Number of augmented views per sample"
    )
    parser.add_argument(
        "--exclude_anchor",
        action="store_true",
        help="Exclude anchor from denominator in MPSQCLLoss",
    )
    ...
    parser.add_argument(
        "--noise_prob", type=float, default=0.0, help="NISQ noise probability"
    )
    parser.add_argument(
    ...
    print(
        f"\nMPSQCL HAR (Paper Version) — Dataset: {args.dataset.upper()} | Phase: {args.phase.upper()}"
    )
