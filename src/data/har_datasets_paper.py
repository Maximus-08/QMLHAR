...
class UnifiedHARDataset(Dataset):
    ...
    def __init__(self, X, y, transform=None):
        ...
        if isinstance(y, np.ndarray) and y.dtype != np.int64:
            y = y.astype(np.int64)
...
def load_uci_har_all(data_dir):
    ...
    if not os.path.exists(uci_dir):
        raise FileNotFoundError(
            f"UCI-HAR Dataset not found at {uci_dir}. Please run extract_dataset.py first."
        )
...
def load_unimib_shar_all(data_dir):
    ...
    sorted_subjects = sorted(
        subject_unique_classes.keys(), key=lambda s: len(subject_unique_classes[s])
    )
    disregarded_subjects = sorted_subjects[:10]
    print(
        f"Programmatically disregarding 10 participants with incomplete classes: {disregarded_subjects}"
    )
    ...
    print(
        f"SHAR loaded. Shape X: {X.shape}, y: {y.shape}, Unique classes: {len(unique_classes)}"
    )
    return X, y
...
def preprocess_hhar(data_dir, output_path):
    ...
    df_acc["key"] = (
        df_acc["User"].astype(str)
        + "_"
        + df_acc["Model"].astype(str)
        + "_"
        + df_acc["Device"].astype(str)
        + "_"
        + df_acc["label"].astype(str)
    )
    df_gyro["key"] = (
        df_gyro["User"].astype(str)
        + "_"
        + df_gyro["Model"].astype(str)
        + "_"
        + df_gyro["Device"].astype(str)
        + "_"
        + df_gyro["label"].astype(str)
    )
    ...
    df_acc_grouped = {
        k: v.sort_values("Creation_Time") for k, v in df_acc.groupby("key")
    }
    df_gyro_grouped = {
        k: v.sort_values("Creation_Time") for k, v in df_gyro.groupby("key")
    }
    ...
    for idx, key in enumerate(sorted(list(keys))):
        ...
        X_seg, y_seg = segment_sliding_window(
            sig, y_continuous, window_size=100, overlap_ratio=0.5
        )
    ...
    if len(X_list) == 0:
        raise ValueError(
            "HHAR preprocessing resulted in empty dataset. Check labels matching."
        )
...
def load_motionsense_all(data_dir):
    ...
    if not os.path.exists(cache_path):
        raise FileNotFoundError(
            f"MotionSense cache not found: {cache_path}. Run download_and_preprocess_datasets.py first."
        )
    data = np.load(cache_path)
    return data["X"], data["y"]
...
def load_uschad_all(data_dir):
    ...
    if not os.path.exists(cache_path):
        raise FileNotFoundError(
            f"USC-HAD cache not found: {cache_path}. Run download_and_preprocess_datasets.py first."
        )
    data = np.load(cache_path)
    return data["X"], data["y"]
...
def load_mobiact_all(data_dir):
    ...
    if not os.path.exists(cache_path):
        raise FileNotFoundError(
            f"MobiAct cache not found: {cache_path}. Run download_and_preprocess_datasets.py first."
        )
    data = np.load(cache_path)
    return data["X"], data["y"]


def preprocess_opportunity(data_dir, task="locomotion"):
    import pandas as pd
    import numpy as np

    print(f"Opportunity Preprocessing Pipeline Started for task '{task}'...")
    opp_dir = os.path.join(data_dir, "OpportunityUCIDataset", "dataset")
    if not os.path.exists(opp_dir):
        raise FileNotFoundError(f"Opportunity dataset directory not found at {opp_dir}")

    # Find all .dat files
    files = sorted([os.path.join(opp_dir, f) for f in os.listdir(opp_dir) if f.endswith(".dat")])
    print(f"Found {len(files)} files to preprocess.")

    # Indices of the 113 body-worn channels
    # 24 columns of Accelerometers (indices 13 to 36)
    # 65 columns of IMUs (indices 37 to 101)
    # 12 columns of L-SHOE: Nav_Ax to AngVelNavFrameZ (indices 105 to 116)
    # 12 columns of R-SHOE: Nav_Ax to AngVelNavFrameZ (indices 121 to 132)
    feature_indices = (
        list(range(13, 37)) +
        list(range(37, 102)) +
        list(range(105, 117)) +
    ...
    assert len(feature_indices) == 113, f"Expected 113 channels, got {len(feature_indices)}"

    label_idx = 243 if task == "locomotion" else 249
    ...
    for idx, filepath in enumerate(files):
        print(f"  Processing file {idx+1}/{len(files)}: {os.path.basename(filepath)}...")
        df = pd.read_csv(filepath, sep=r"\s+", header=None)
        
        # 1. Linearly interpolate NaNs inside feature columns, then fill remaining NaNs with 0
        features = df[feature_indices]
        features = features.interpolate(method="linear", limit_direction="both").fillna(0.0)
        X_file = features.values
        
        # 2. Extract labels
        y_file = df[label_idx].fillna(0).values
        
        # 3. Pre-map label sequence to avoid np.bincount array-allocation issues inside segment_sliding_window
        if task == "locomotion":
            # Map 1 -> 1, 2 -> 2, 4 -> 4, 5 -> 5, others -> 0
            mapping = {1: 1, 2: 2, 4: 4, 5: 5}
            y_mapped = np.array([mapping.get(val, 0) for val in y_file])
        else:
            gesture_classes = [
                0, 406516, 406517, 404516, 404517, 406520, 404520, 406505,
                404505, 406519, 404519, 406511, 404511, 406508, 404508,
                408512, 407521, 405506
            ]
            mapping = {val: i for i, val in enumerate(gesture_classes)}
            y_mapped = np.array([mapping.get(val, 0) for val in y_file])
            
        # 4. Slide window segment
        X_seg, y_seg = segment_sliding_window(X_file, y_mapped, window_size=window_size, overlap_ratio=overlap_ratio)
        
        if len(X_seg) > 0:
            if task == "locomotion":
                # Filter out Null locomotion class (0)
                mask = (y_seg != 0)
                X_seg = X_seg[mask]
                y_seg = y_seg[mask]
                
                # Map 1 -> 0, 2 -> 1, 4 -> 2, 5 -> 3
                final_mapping = {1: 0, 2: 1, 4: 2, 5: 3}
                y_seg = np.array([final_mapping[val] for val in y_seg])
    ...
    if len(X_list) == 0:
        raise ValueError(f"Opportunity preprocessing for task '{task}' resulted in empty dataset.")

    X = np.concatenate(X_list, axis=0)
    y = np.concatenate(y_list, axis=0)

    # Global channel-wise normalization
    print(f"Applying global channel-wise normalization to Opportunity ({task})...")
    mean = np.mean(X, axis=(0, 2), keepdims=True)
    std = np.std(X, axis=(0, 2), keepdims=True)
    std[std == 0.0] = 1.0
    X = (X - mean) / std
    ...
    # Save to file
    out_dir = os.path.join(data_dir, "Opportunity")
    os.makedirs(out_dir, exist_ok=True)
    filename = "opportunity_preprocessed.npz" if task == "locomotion" else "opportunity_gestures_preprocessed.npz"
    cache_path = os.path.join(out_dir, filename)
    print(f"Saving preprocessed dataset to {cache_path}...")
    np.savez_compressed(cache_path, X=X, y=y)
    print("Opportunity Preprocessing Complete!")
    return X, y


def load_opportunity_all(data_dir, task="locomotion"):
    filename = "opportunity_preprocessed.npz" if task == "locomotion" else "opportunity_gestures_preprocessed.npz"
    cache_path = os.path.join(data_dir, "Opportunity", filename)
    if os.path.exists(cache_path):
        print(f"Loading preprocessed Opportunity ({task}) from cache: {cache_path}")
        data = np.load(cache_path)
        X = data["X"].astype(np.float32)
        y = data["y"]
        data.close()
        return X, y
    X, y = preprocess_opportunity(data_dir, task)
...
def get_paper_dataloaders(
    ...
    if dataset_name == "ucihar":
        ...
        X, y = load_opportunity_all(data_dir, task="locomotion")
    elif dataset_name == "opportunity_gestures":
        X, y = load_opportunity_all(data_dir, task="gestures")
    else:
        raise ValueError(
            f"Unknown dataset name: {dataset_name}. Choose 'ucihar', 'shar', 'hhar', 'motionsense', 'uschad', 'mobiact', 'opportunity', or 'opportunity_gestures'."
        )
...
if __name__ == "__main__":
    ...
    try:
        tr, va, te, ch, w, classes = get_paper_dataloaders(
            "ucihar", data_dir, batch_size=32
        )
