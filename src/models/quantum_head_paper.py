"""
Paper-compliant Quantum Projection Head for contrastive learning.

Uses AmplitudeEmbedding to encode 256-dimensional feature vectors into 8-qubit
quantum states, followed by a custom PQC with Ry rotation gates and adjacent CNOTs
for variational processing, and PauliZ measurements for the output projection.

Includes NISQ noise simulation with Rx rotation gates.
"""
...
if __name__ == "__main__":
    head = QuantumProjectionHeadPaper(input_dim=256, num_qubits=8, q_layers=1)
    ...
    head.set_noise(noise_prob=0.7, noise_std=0.2)
    out_noise = head(dummy)
